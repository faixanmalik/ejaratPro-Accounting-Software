...
import useTranslation from 'next-translate/useTranslation';
  ...
  const CreditSalesInvoice = ({ locale, isLoading, setIsLoading, userEmail, dbVouchers, dbProducts, dbContacts, dbEmployees, dbTaxRate }) => {
    ...
    const handleSearch = (e) => {
      ...
      if (inputValue != '') {
        filtered = filtered.filter(item => {
          return item.billNo.toLowerCase().includes(inputValue.toLowerCase())
        });
      }
            ...
            <div className='flex items-center space-x-2 rtl:space-x-reverse mb-1'>
              <div className=''>
                ...
                <Transition.Root show={openModal} as={Fragment}>
                  <Dialog as="div" dir={`${locale === 'ar' && 'rtl'}`} className="relative z-10" initialFocus={cancelButtonRef} onClose={setOpenModal}>
                    <Transition.Child
                      as={Fragment}
